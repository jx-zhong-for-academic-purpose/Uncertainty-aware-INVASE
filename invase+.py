...
class PVS():
    ...
    def build_generator(self):
        ...
        # model.add(Lambda(lambda x: tf.add(x, 0.95 - x)))
...
if __name__ == '__main__':
    ...
    #print("Pearson Correlation Coefficient: ", np.corrcoef(x, y)[0][1])
    import matplotlib.pyplot as plt
    text = "x: log(var), y:dis_pred cross entropy"
    plt.figure()
    pred_mse = (2 * np.square((np.array(y_true) - np.array(y_pred)))).flatten().tolist()
    plt.scatter(log_var, pred_mse)
    plt.title("%s" % text)
    plt.show()

    import pickle
    with open("log_var.pkl", "wb") as f:
    ...
    Sel_Prob = PVS_Alg.output(x_test)
